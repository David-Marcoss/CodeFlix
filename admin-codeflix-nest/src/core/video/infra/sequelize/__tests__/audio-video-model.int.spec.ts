...
describe('AudioVideoMediaModel Unit Tests', () => {
  ...
  test('mapping props', () => {
    ...
    expect(uniqueIndex).toMatchObject({
      fields: ['video_id', 'audio_video_related_field'],
      unique: true,
    });
    ...
    const videoRelatedFieldAttr = attributesMap.audio_video_related_field;
    expect(videoRelatedFieldAttr).toMatchObject({
      field: 'audio_video_related_field',
      fieldName: 'audio_video_related_field',
      allowNull: false,
