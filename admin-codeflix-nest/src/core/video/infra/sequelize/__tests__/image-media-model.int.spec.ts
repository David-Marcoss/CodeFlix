...
describe('ImageMediaModel Unit Tests', () => {
  ...
  test('mapping props', () => {
    ...
    expect(uniqueIndex).toMatchObject({
      fields: ['video_id', 'image_media_related_field'],
      unique: true,
    });
    ...
    const videoRelatedFieldAttr = attributesMap.image_media_related_field;
    expect(videoRelatedFieldAttr).toMatchObject({
      field: 'image_media_related_field',
      fieldName: 'image_media_related_field',
      allowNull: false,
      type: DataType.ENUM(
