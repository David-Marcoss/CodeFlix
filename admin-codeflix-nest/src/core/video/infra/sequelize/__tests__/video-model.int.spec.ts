...
describe('VideoGenresModel Unit Tests', () => {
  setupSequelizeForVideo();

  test('table name', () => {
    expect(VideoGenresModel.tableName).toBe('genre_video');
  });

  test('mapping props', () => {
    const attributesMap = VideoGenresModel.getAttributes();
    const attributes = Object.keys(VideoGenresModel.getAttributes());
...
describe('VideoModel Unit Tests', () => {
  const database = setupSequelizeForVideo();
  ...
  test('mapping associations', () => {
    ...
    expect(genresRelation).toMatchObject({
      ...
      target: GenreModel,
      options: {
        through: { model: VideoGenresModel },
        foreignKey: { name: 'video_id' },
  ...
  test('create and association relations separately ', async () => {
    ...
    const genreRepo = new GenreSequelizeRepository(
      GenreModel,
      new UnitOfWorkSequelize(database.sequelize),
    );
    ...
    expect(videoWithGenres!.genres_id[0]).toBeInstanceOf(VideoGenresModel);
  ...
  test('create and association in single transaction ', async () => {
    ...
    expect(videoWithRelations!.genres_id[0]).toBeInstanceOf(VideoGenresModel);
