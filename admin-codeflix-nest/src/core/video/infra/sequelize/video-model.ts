...
export class VideoModel extends Model<VideoModelProps> {
  ...
  @HasMany(() => VideoGenresModel, 'video_id')
  declare genres_id: VideoGenresModel[];

  @BelongsToMany(() => GenreModel, () => VideoGenresModel)
...
export interface VideoGenresModelProps {
  video_id: string;
  genre_id: string;
}

@Table({ tableName: 'genre_video', timestamps: false })
export class VideoGenresModel extends Model<VideoGenresModelProps> {
  @PrimaryKey
  @ForeignKey(() => VideoModel)
  @Column({ allowNull: false, type: DataType.UUID })
  declare video_id: string;
