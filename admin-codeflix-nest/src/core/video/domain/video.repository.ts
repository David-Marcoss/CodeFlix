...
export class VideoSearchParams extends SearchParams<VideoFilter> {
  ...
  static create(
    ...
    const categories_id = props.filter?.categories_id?.map((c) => {
      return c instanceof VideoId ? c : new VideoId(c);
    });

    const genres_id = props.filter?.genres_id?.map((g) => {
      return g instanceof GenreId ? g : new GenreId(g);
    });

    const cast_members_id = props.filter?.cast_members_id?.map((cm) => {
      return cm instanceof CastMemberId ? cm : new CastMemberId(cm);
    });
...
export interface IVideoRepository extends ISearchableRepository<
  Video,
  VideoId,
  VideoFilter,
  VideoSearchParams,
