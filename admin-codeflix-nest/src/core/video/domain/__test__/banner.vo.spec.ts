...
describe('Banner Unit Tests', () => {
  it('should create a Banner object from a valid file', () => {
    ...
    const banner = Banner.createFromFile({
      raw_name: 'test.png',
      mime_type: 'image/png',
      size: data.length,
      video_id: videoId,
  ...
  it('should throw an error if the file size is too large', () => {
    ...
    expect(() =>
      ...
      new InvalidMediaFileSizeError(Banner.max_size + 1, Banner.max_size),
    );
  });
  ...
  it('should throw an error if the file mime type is not valid', () => {
    ...
    expect(() =>
      ...
      new InvalidMediaFileMimeTypeError('text/plain', Banner.mime_types),
    );
  });
});
