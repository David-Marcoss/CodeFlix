...
describe('GenreSequelizeRepository Integration Tests', () => {
  ...
  it('should creates a new entity', async () => {
    ...
    const newGenre = await genreRepo.getById(genre.genre_id);
    expect(newGenre!.toJSON()).toStrictEqual(genre.toJSON());
  });

  it('should bulk creates new entities', async () => {
    ...
    const newGenres = await genreRepo.getAll();
  ...
  it('should finds a entity by id', async () => {
    ...
    const entityFound = await genreRepo.getById(genre.genre_id);
    expect(genre.toJSON()).toStrictEqual(entityFound!.toJSON());

    await expect(genreRepo.getById(new GenreId())).resolves.toBeNull();
  });
  ...
  it('should return all categories', async () => {
    ...
    const entities = await genreRepo.getAll();
  ...
  it('should update a entity', async () => {
    ...
    let entityFound = await genreRepo.getById(genre.genre_id);
    expect(genre.toJSON()).toStrictEqual(entityFound!.toJSON());
    ...
    entityFound = await genreRepo.getById(genre.genre_id);
  ...
  describe('transaction mode', () => {
    describe('create method', () => {
      it('should create a genre', async () => {
        ...
        const result = await genreRepo.getById(genre.genre_id);
    ...
    describe('createMany method', () => {
      it('should create a list of genres', async () => {
        const category = Category.fake().aCategory().build();
        await categoryRepo.create(category);
        const genres = Genre.fake()
          .theGenres(2)
        ...
        const [genre1, genre2] = await Promise.all([
          genreRepo.getById(genres[0].genre_id),
          genreRepo.getById(genres[1].genre_id),
        ]);
      ...
      it('rollback the bulk createion', async () => {
        ...
        await expect(genreRepo.getById(genres[0].genre_id)).resolves.toBeNull();
        await expect(genreRepo.getById(genres[1].genre_id)).resolves.toBeNull();
      });
    });
    ...
    describe('.getAll method', () => {
      it('should return a list of genres', async () => {
        ...
        await uow.start();
        await genreRepo.createMany(genres);
        const result = await genreRepo.getAll();
        expect(result.length).toBe(2);
        await uow.commit();
      });
    });

    // describe('.getByIds method', () => {
    //   it('should return a list of genres', async () => {
    //     const category = Category.fake().aCategory().build();
    //     await categoryRepo.create(category);
    //     const genres = Genre.fake()
    ...
    //     await uow.start();
    //     await genreRepo.createMany(genres);
    //     const result = await genreRepo.getByIds(genres.map((g) => g.genre_id));
    //     expect(result.length).toBe(2);
    //     await uow.commit();
    //   });
    // });
    ...
    describe('update method', () => {
      ...
      it('rollback the update', async () => {
        ...
        await genreRepo.update(genre);
        await uow.rollback();
        const notChangeGenre = await genreRepo.getById(genre.genre_id);
